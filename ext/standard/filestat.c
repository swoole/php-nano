...
{
#if defined(PHP_NANO) && defined(__wasi__)
	(void) name;
	(void) gid;
	return FAILURE;
#else
...
/* On Windows, we expect regular chgrp to fail silently by default */
	...
	/* WASI has no process user/group identity or chown operation. */
	RETURN_FALSE;
#else
	if (group_str) {
...
PHPAPI void php_stat(zend_string *filename, int type, zval *return_value)
{
	...
	case FS_TYPE:
		...
		if (S_ISBLK(stat_sb->st_mode)) RETURN_STRING("block");
		if (S_ISREG(stat_sb->st_mode)) RETURN_STR(ZSTR_KNOWN(ZEND_STR_FILE));
# if defined(S_ISSOCK) && !defined(PHP_WIN32)
		if (S_ISSOCK(stat_sb->st_mode)) RETURN_STRING("socket");
# endif
		php_error_docref(NULL, E_NOTICE, "Unknown file type (%d)", stat_sb->st_mode);
#else
