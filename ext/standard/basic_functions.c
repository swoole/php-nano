...
{
#ifdef ZTS
	ts_allocate_id(&basic_globals_id, sizeof(php_basic_globals), (ts_allocate_ctor) basic_globals_ctor, NULL);
#else
...
PHP_RINIT_FUNCTION(basic)
{
	...
	if (PHP_RINIT(filestat)(INIT_FUNC_ARGS_PASSTHRU) != SUCCESS) {
		return FAILURE;
	}
	...
	ZEND_HASH_MAP_FOREACH_STR_KEY_PTR(EG(ini_directives), key, ini_entry) {
		...
		if (key == NULL || ZSTR_VAL(key)[0] != 0) {
			if (details) {
				...
				zval value;
				ZVAL_STR_COPY(&value, ini_entry->value);
				zend_symtable_update(Z_ARRVAL_P(return_value), ini_entry->name, &value);
			} else {
...
/* Kept byte-for-byte equivalent to the upstream implementation below. */
