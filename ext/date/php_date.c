...
{
#ifdef PHP_NANO
	return (time_t) php_nano_system_time_seconds();
#else
...
{
#ifdef PHP_NANO
	/* phpinfo() and its table renderer belong to the SAPI-facing layer. */
	return;
#else
...
#ifdef PHP_NANO
typedef int php_date_microseconds_t;
#else
typedef suseconds_t php_date_microseconds_t;
#endif

static void php_date_get_current_time_with_fraction(time_t *sec, php_date_microseconds_t *usec)
{
#ifdef PHP_NANO
	int64_t nano_seconds;
	int32_t nano_microseconds;
	php_nano_system_time(&nano_seconds, &nano_microseconds);
	*sec = (time_t) nano_seconds;
	*usec = (php_date_microseconds_t) nano_microseconds;
#else
#ifdef HAVE_GETTIMEOFDAY
