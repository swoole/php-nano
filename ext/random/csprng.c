...
#if !defined(PHP_WIN32) && !defined(PHP_NANO)
...
{
#ifdef PHP_NANO
	if (php_nano_random_bytes(bytes, size) == FAILURE) {
		snprintf(errstr, errstr_size, "C++ standard library entropy source is unavailable");
		return FAILURE;
	}
