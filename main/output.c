...
{
#ifdef PHP_NANO
	/* Nano has no SAPI or HTTP header layer. Output is sent directly to the
	 * host writer, while the ordinary PHP output-buffer stack remains active. */
	return;
#else
	if (!SG(headers_sent)) {
...
#endif
			php_output_direct(context.out.data, context.out.used);
