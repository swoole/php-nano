...
ZEND_API ZEND_COLD void zend_throw_exception_internal(zend_object *exception) /* {{{ */
{
	...
	if (!EG(current_execute_data)) {
		...
		if (EG(exception)) {
#ifdef PHP_NANO
			/* TypePHP AOT functions call one another directly and therefore do
			 * not require a synthetic Zend execute frame. Leave the exception in
			 * EG(exception); the PHPX boundary immediately converts it to the
			 * C++ zend_object* unwind used by generated try/catch code. */
			return;
#else
