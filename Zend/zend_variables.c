...
#ifdef PHP_NANO
	/* A Nano artifact cannot create constant AST zvals because it has no
	 * parser or runtime compiler. Keep the zval type slot for ABI layout. */
	[IS_CONSTANT_AST] = (zend_rc_dtor_func_t)zend_empty_destroy
#else
