...
#ifdef PHP_NANO
# define zend_quiet_write(fd, buffer, length) \
	((void) fwrite((buffer), 1, (length), (fd) == 2 ? stderr : stdout))
#else
...
#if !defined(PHP_NANO) && !ZEND_DEBUG && (defined(HAVE_ALLOCA) || (defined (__GNUC__) && __GNUC__ >= 2)) && !(defined(ZTS) && defined(HPUX)) && !defined(__APPLE__)
...
#elif !defined(PHP_NANO) && (__STDC_VERSION__ >= 202311L || ZEND_GCC_VERSION)
