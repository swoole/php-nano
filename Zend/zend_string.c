 ...
 * aligned allocation but intentionally left uninitialized.  Nano uses the
 * bounded standard-library implementation because its malloc backend does not
 * carry the Valgrind replacement symbol through final executable section GC.
 */
...
#if !defined(PHP_NANO) && defined(__GNUC__) && defined(__i386__)
...
#elif !defined(PHP_NANO) && defined(__GNUC__) && defined(__x86_64__) && !defined(__ILP32__)
ZEND_API zend_never_inline NOIPA bool ZEND_FASTCALL zend_string_equal_val(const zend_string *s1, const zend_string *s2)
{
	const char *ptr = ZSTR_VAL(s1);
	uintptr_t delta = (uintptr_t) s2 - (uintptr_t) s1;
