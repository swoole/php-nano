...
#elif defined(__SANITIZE_ADDRESS__) && !defined(PHP_NANO)
...
#ifdef PHP_NANO
		/* Constant AST values only originate in the PHP source compiler. */
		return FAILURE;
#else
...
#endif /* !PHP_NANO: php-nano dispatches AOT/internal handlers in PHPX. */
...
#endif /* !PHP_NANO: execution timers require OS signal/timer APIs. */
